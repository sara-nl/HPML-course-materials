...
# Simple Train
def train(args, model, device, train_loader, optimizer, epoch, idx_profile_start, idx_profile_end):
    model.train()
...
def main():
    ...
    parser.add_argument('--profile-start-idx',type=int, default=10,
                        help='Starting index for profiling')
    parser.add_argument('--profile-end-idx',type=int, default=15,
                        help='End index for profiling')
    ...
    if TEACHER_DIR is not None:
        MNIST_DATA = os.path.join(TEACHER_DIR, '/JHS_data')
    else:
        MNIST_DATA = os.path.join(os.getenv('HOME'), 'MNIST_DATA')
    ...
    for epoch in range(1, args.epochs + 1):
        train(args, ddp_model, local_rank, train_loader, optimizer, epoch, args.profile_start_idx, args.profile_end_idx)
